...
function play() {
    ...
    .then(response => {
        allQuesArray = response.results.map(q => {
            let allQuesObject = {};
            allQuesObject.question = q.question;
            allQuesObject.answer = Math.floor(Math.random() * 3) + 1;
            const choices = [...q.incorrect_answers];
            choices.splice(allQuesObject.answer - 1, 0, q['correct_answer']);
            choices.forEach((eachChoice, index) => {
                allQuesObject["choice" + (index + 1)] = eachChoice;
            });
            return allQuesObject;
            });
        startGame();
    })
}
...
function startGame() {
    score = 0;
    questionCounter = 0;
    remainedQuestions = [...allQuesArray];
    getQuestion();
